...
- No private source imported. Local policy lives separately in `tools/oxlint/aha-policy/` and is not upstream code.
...
- `tools/oxlint/aha-policy/index.ts` implements `aha-policy/*` rules:
