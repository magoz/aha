...
export function renderScalableList(
  ...
  return `<div class="aha-scale" data-scale="scalable-list" data-scale-id="${escapeAttr(options.idPrefix)}" data-serves="${escapeAttr(String(input.serves))}" data-label="${escapeAttr(label)}">${title}<p class="sc-yield"><span class="sc-cur">${escapeHtml(String(input.serves))} ${escapeHtml(label)}</span></p><div class="sc-ctl" hidden><button type="button" data-step="-1" aria-label="Serve one fewer">−</button><output>${escapeHtml(String(input.serves))} ${escapeHtml(label)}</output><button type="button" data-step="1" aria-label="Serve one more">+</button>${presetWrap}</div><ul class="sc-items">${items}</ul></div>`
}
