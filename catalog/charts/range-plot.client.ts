...
import { decodeRangePlotJson } from './range-plot-codec.js'
import { RANGE_PLOT_WIDTH, rangePlotStops, renderRangePlot } from './range-plot-render.js'
import type { RangePlotInput } from './range-plot-schema.js'

/**
 * Range-plot browser client. Compact IIFE entry built by the repo build;
 * `aha build` inlines the bundle. No Effect or Schema here: the JSON was
 * validated at build time and is read defensively.
 */

function readInput(figure: HTMLElement): RangePlotInput | null {
  ...
  return decodeRangePlotJson(record)
}
...
function stopsFor(figure: HTMLElement): ReadonlyArray<ChartStop> {
  ...
  return rangePlotStops(input, currentWidth(figure))
}
...
function rerender(figure: HTMLElement): void {
  ...
  const html = renderRangePlot(input, {
...
function initRangePlots(): void {
  const figures = document.querySelectorAll('figure[data-aha="range-plot"]')
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initRangePlots)
} else {
  initRangePlots()
}
