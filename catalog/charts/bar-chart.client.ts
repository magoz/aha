...
import { decodeBarChartJson } from './bar-chart-codec.js'
import { BAR_CHART_WIDTH, barChartStops, renderBarChart } from './bar-chart-render.js'
import type { BarChartInput } from './bar-chart-schema.js'

/**
 * Bar-chart browser client. Compact IIFE entry built by the repo build;
 * `aha build` inlines the bundle. No Effect or Schema here: the JSON was
 * validated at build time and is read defensively.
 */

function readInput(figure: HTMLElement): BarChartInput | null {
  ...
  return decodeBarChartJson(record)
}
...
function stopsFor(figure: HTMLElement): ReadonlyArray<ChartStop> {
  ...
  return barChartStops(input, currentWidth(figure))
}
...
function rerender(figure: HTMLElement): void {
  ...
  const html = renderBarChart(input, {
...
function initBarCharts(): void {
  const figures = document.querySelectorAll('figure[data-aha="bar-chart"]')
