...
import { decodeHistogramJson } from './histogram-codec.js'
import { HISTOGRAM_WIDTH, histogramStops, renderHistogram } from './histogram-render.js'
import type { HistogramInput } from './histogram-schema.js'

/**
 * Histogram browser client. Compact IIFE entry built by the repo build;
 * `aha build` inlines the bundle. No Effect or Schema here: the JSON was
 * validated at build time and is read defensively.
 */

function readInput(figure: HTMLElement): HistogramInput | null {
  ...
  return decodeHistogramJson(record)
}
...
function stopsFor(figure: HTMLElement): ReadonlyArray<ChartStop> {
  ...
  return histogramStops(input, currentWidth(figure))
}
...
function rerender(figure: HTMLElement): void {
  ...
  const html = renderHistogram(input, {
...
function initHistograms(): void {
  const figures = document.querySelectorAll('figure[data-aha="histogram"]')
