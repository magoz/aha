...
import { TIME_STRIPS_WIDTH, renderTimeStrips, timeStripsStops } from './time-strips-render.js'
import type { TimeStripsInput } from './time-strips-schema.js'

/**
 * Time-strips browser client. Compact IIFE entry built by the repo build;
 * `aha build` inlines the bundle. No Effect or Schema here.
 */

function readInput(figure: HTMLElement): TimeStripsInput | null {
  ...
  return decodeTimeStripsJson(record)
}
...
function stopsFor(figure: HTMLElement): ReadonlyArray<ChartStop> {
  ...
  return timeStripsStops(input, currentWidth(figure))
}
...
function rerender(figure: HTMLElement): void {
  ...
  const html = renderTimeStrips(input, {
...
function initTimeStrips(): void {
  const figures = document.querySelectorAll('figure[data-aha="time-strips"]')
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initTimeStrips)
} else {
  initTimeStrips()
}
