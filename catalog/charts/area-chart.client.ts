...
import { decodeAreaChartJson } from './area-chart-codec.js'
import { AREA_CHART_WIDTH, areaChartStops, renderAreaChart } from './area-chart-render.js'
import type { AreaChartInput } from './area-chart-schema.js'

/**
 * Area-chart browser client. Compact IIFE entry built by the repo build;
 * `aha build` inlines the bundle. No Effect or Schema here: the JSON was
 * validated at build time and is read defensively.
 */

function readInput(figure: HTMLElement): AreaChartInput | null {
  ...
  return decodeAreaChartJson(record)
}
...
function stopsFor(figure: HTMLElement): ReadonlyArray<ChartStop> {
  ...
  return areaChartStops(input, currentWidth(figure))
}
...
function rerender(figure: HTMLElement): void {
  ...
  const html = renderAreaChart(input, {
...
function initAreaCharts(): void {
  const figures = document.querySelectorAll('figure[data-aha="area-chart"]')
