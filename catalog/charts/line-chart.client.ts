...
import { decodeLineChartJson } from './line-chart-codec.js'
import { LINE_CHART_WIDTH, lineChartStops, renderLineChart } from './line-chart-render.js'
import type { LineChartInput } from './line-chart-schema.js'

/**
 * Line-chart browser client. Compact IIFE entry built by the repo build;
 * `aha build` inlines the bundle. No Effect or Schema here: the JSON was
 * validated at build time and is read defensively.
 */

function readInput(figure: HTMLElement): LineChartInput | null {
  ...
  return decodeLineChartJson(record)
}
...
function stopsFor(figure: HTMLElement): ReadonlyArray<ChartStop> {
  ...
  return lineChartStops(input, currentWidth(figure))
}
...
function rerender(figure: HTMLElement): void {
  ...
  const html = renderLineChart(input, {
...
function initLineCharts(): void {
  const figures = document.querySelectorAll('figure[data-aha="line-chart"]')
