...
import { decodeScatterPlotJson } from './scatter-plot-codec.js'
import { SCATTER_PLOT_WIDTH, renderScatterPlot, scatterPlotStops } from './scatter-plot-render.js'
import type { ScatterPlotInput } from './scatter-plot-schema.js'

/**
 * Scatter-plot browser client. Compact IIFE entry built by the repo build;
 * `aha build` inlines the bundle. No Effect or Schema here: the JSON was
 * validated at build time and is read defensively.
 */

function readInput(figure: HTMLElement): ScatterPlotInput | null {
  ...
  return decodeScatterPlotJson(record)
}
...
function stopsFor(figure: HTMLElement): ReadonlyArray<ChartStop> {
  ...
  return scatterPlotStops(input, currentWidth(figure))
}
...
function rerender(figure: HTMLElement): void {
  ...
  const html = renderScatterPlot(input, {
...
function initScatterPlots(): void {
  const figures = document.querySelectorAll('figure[data-aha="scatter-plot"]')
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initScatterPlots)
} else {
  initScatterPlots()
}
